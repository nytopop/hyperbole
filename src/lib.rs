...
mod tests {
    ...
    async fn get_body<I: Clone, B: Into<Body>>(
        app: &App<I>,
        path: &str,
        body: B,
    ) -> (Parts, String)
    {
        ...
        let uri = format!("http://any{}", path);

        let req = Request::get(uri).body(body.into()).unwrap();
        let resp = app.dispatch(req, LOCAL.into()).await;

        let (parts, body) = resp.into_parts();

        let raw = to_bytes(body).await.unwrap();

        (parts, String::from_utf8_lossy(&*raw).into_owned())
    }

    async fn get<I: Clone>(app: &App<I>, path: &str) -> (Parts, String) {
        get_body(app, path, "").await
    }
    ...
    async fn test_basic_uri_parsing() {
        ...
        let (p, body) = get(&app, "/44/55").await;
        assert_eq!("x: 44, y: 55", body);
    ...
    async fn test_route_dispatch_with_ctx_val() {
        ...
        for (path, val) in vec![
            ...
            let (p, body) = get(&app, path).await;

            assert_eq!(format!("{}", val), body);
            assert!(p.status.is_success());
        }
    }
}
